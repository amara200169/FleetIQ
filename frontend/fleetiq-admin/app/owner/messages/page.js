      ...
      <div className="h-full flex">
        ...
        <div className="w-72 bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {contacts.length === 0 && (
              <p className="p-4 text-sm text-gray-400">No drivers found.</p>
            )}
            {contacts.map((c) => (
              <button key={c.id} onClick={() => openConversation(c)}
                className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition ${selected?.id === c.id ? 'bg-blue-50' : ''}`}>
                <div className="flex justify-between items-start">
                  ...
                  {c.unreadCount > 0 && (
                    <span className="ml-2 bg-blue-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0">{c.unreadCount}</span>
                  )}
                </div>
              </button>
            ))}
